...
module.exports = (() => {
  ...
  const reportFeedbacks = (event, callback) => {
    ...
    if (event.accountid && event.surveyid && process.env.SERVERLESS_FEEDBACKTABLE) {
      ...
      const getSurveyFormat = new Promise( (resolve, reject) => {
        survey.getOneSurvey({
          ...
          if (err) {
            ...
            let feedbackFieldNames = [];
            let feedbackField = [];
            /** Json2CSV format can refer (https://github.com/zemirco/json2csv). **/
            // mapping to csv2json format
            for (let i = 0; i < response['survey']['content'].length ; i++) {
              for (let k = 0; k < response['survey']['content'][i]['question'].length; k++){
                let question = response['survey']['content'][i]['question'][k];

                if (question['type'] === 'radio' || question['type'] === 'rating') {
                  feedbackFieldNames.push(question.label);
                  feedbackField.push(`feedback.Q${question.order}.data[0].label`);
                  if (question['input']) {
                    feedbackFieldNames.push(question.input);
                    feedbackField.push(`feedback.Q${question.order}.data[0].input`);
                  }
                } else if (question['type'] === 'textarea' || question['type'] === 'input') {
                  feedbackFieldNames.push(question.label);
                  feedbackField.push(`feedback.Q${question.order}.data[0].input`);
                } else if (question['type'] === 'checkbox') {
                  for (let j = 0 ; j < question.data.length ; j++) {
                    feedbackFieldNames.push(`${question.label}(${j+1})`);
                    feedbackField.push(`feedback.Q${question.order}.data[${j}].label`);
                    if (question['data'][j]['input']) {
                      feedbackFieldNames.push(`${question['data'][j]['input']}(${j+1})`);
                      feedbackField.push(`feedback.Q${question.order}.data[${j}].input`);
                    }
                  }
                }
              }
      ...
      getSurveyFormat.then( (format) => {
        return dynamoQuery([]).then( (feedbackData) => {
          return {
            "feedbacksData" : feedbackData[0],
            "feedbackField" : format.feedbackField,
            "feedbackFieldNames" : format.feedbackFieldNames
          };
        });
      }).then( (csvData) => {
        try {
          ...
          console.error(err);
          return callback(new Error("400 Bad Request: json to csv error: " + JSON.stringify(err)), null);
        }
        return callback(null, response);
      }).catch( (err) => {
